...
class M3RecognitionItemTest {
    ...
    fun `the answer is one of the choices, and the label points at it`() {
        for (skill in patternChoiceNodes) {
            for (seed in 1L..25L) {
                ...
                assertEquals((q.answerIndex + 1).toString(), item.correctLabel)
            }
        }
    }
    ...
    @Test
    fun `tolerance level decides how many choices are offered`() {
        ...
        fun choices(level: Int) =
            (1L..20L).map {
                val q =
                    generate(
                        SkillIds.M3_SUBDIV_RECOG,
                        levels(DifficultyAxis.TIMING_TOLERANCE to level),
                        it,
                    ).question
                assertIs<RhythmQuestion.WhichPattern>(q)
                q.choices.size
            }
        assertTrue(choices(0).all { it == 2 }, "level 0 should offer two")
        assertTrue(choices(3).all { it == 3 }, "level 3 should offer three")
    }
